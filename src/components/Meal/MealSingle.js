...
const MealSingle = ({ meal }) => {
  console.log(meal);
  let tags = meal?.tags?.split(','); //imparte sirul de tag uri intr un array,folosinf virgula
  console.log(meal);
  let instructions = meal?.instructions?.split('\r\n');
  instructions = instructions?.filter(instruction => instruction.length > 1); //instruct goale sunt eliminate prin filter
...
//Operatorul de accesare opțională ?. este utilizat pentru a preveni erorile în cazul în care tags este null sau undefined
